...
#define MOV(a,b,c,d,t) (float2(a*cos(t)+b*cos(0.1*(t)), c*sin(t)+d*cos(0.1*(t))))
			...
			float circle3(float2 uv, float2 center, float radius, float width)
			{
				...
				return smoothstep(2.0, 2.1, abs(mod(theta + 2.0, 45.0) - 2.0)) *
					lerp(0.5, 1.0, step(45.0, abs(mod(theta, 180.0) - 90.0))) *
			...
			{
				float r = length(uv - center);
				float R = 8.0 + mod(87.0 * _Time.y, 80.0);
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				//center of the image
				float2 c = uv / 2.0;
				finalColor = 0.3 * _cross(uv, c, 240.0);
				...
				if (length(uv - c) < 240.0)
				{
					...
					float2 p = 130.0 * MOV(1.3, 1.0, 1.0, 1.4, 3.0 + 0.1 * _Time.y);
